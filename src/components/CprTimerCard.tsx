...
export function CprTimerCard({
  ...
  return (
    <div
      className="bg-slate-900 rounded-xl border border-slate-800 p-3 sm:p-4 flex flex-col items-center justify-between relative overflow-hidden shadow-xl shrink-0 min-h-[400px] sm:h-[450px]"
    >
      ...
      <div className="w-full mb-3">
        <div className="flex items-center p-1 bg-slate-950 rounded-xl border border-slate-800 w-full shadow-inner">
          <button
            ...
            className={`flex-1 py-1.5 px-2 rounded-lg text-[10px] sm:text-[11px] font-black transition-all cursor-pointer flex items-center justify-center gap-1.5 ${
          ...
          >
            ...
            <span>CPR 30:2 (5 รอบ)</span>
          </button>
        </div>
      </div>

      {/* Timer Digits Display with Circular Progress Gauge */}
      <div style={{ marginTop: '-15px' }} className="flex flex-col items-center justify-center my-1 w-full">
        <div className="relative flex items-center justify-center w-52 h-52 xs:w-56 xs:h-56 sm:w-60 sm:h-60 my-2.5">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center text-center p-2">
            <div
              id="timer-display"
              className={`text-4xl xs:text-5xl sm:text-6xl font-mono font-black leading-none tracking-tight tabular-nums ${
            ...
            <span
              className={`text-[10px] font-bold tracking-wider uppercase mt-2 px-2.5 py-0.5 rounded-full border transition-all ${
                cprActive
                  ? cprTimeRemaining <= 30
            ...
            metronomeMode === '30:2' ? (
              metronomeBeat >= 1 && metronomeBeat <= 30 ? (
                <span className="text-[11px] font-mono font-bold text-emerald-400 flex items-center gap-1">
                  <Heart className="w-3.5 h-3.5 fill-emerald-400 animate-ping" />
              ...
              <span className="text-[11px] font-mono font-bold text-cyan-400 flex items-center gap-1">
                <Heart className="w-3.5 h-3.5 fill-cyan-400 animate-ping" />
        ...
        {metronomeMode === '30:2' && (
          <div className="w-full bg-slate-950/80 border border-slate-800 rounded-lg p-1.5 flex items-center justify-between gap-1 my-1">
            <span className="text-[10px] font-bold text-cyan-400 shrink-0 pl-1">
      ...
      <div className="grid grid-cols-12 gap-2 w-full mt-1">
        ...
        <button
          id="start-btn"
          onClick={toggleCPR}
          className={`col-span-8 h-12 text-white rounded-xl text-base font-bold flex items-center justify-center gap-2 shadow-md transition-all duration-200 active:scale-[0.98] cursor-pointer border ${
            cprActive
        ...
        >
          <Activity className={`w-5 h-5 ${cprActive ? 'animate-bounce' : ''}`} />
          <span className="tracking-wide">{cprActive ? 'PAUSE CPR' : 'START CPR'}</span>
        </button>
        ...
        <button
          ...
          className={`col-span-2 h-12 rounded-xl flex flex-col items-center justify-center transition-all duration-200 active:scale-[0.95] cursor-pointer border backdrop-blur-md shadow-md group relative overflow-hidden ${
            pulseCheckActive
        ...
        >
          <div className="flex items-center justify-center gap-0.5">
            <Clock className={`w-3.5 h-3.5 transition-transform duration-200 ${pulseCheckActive ? 'text-slate-950 stroke-[2.5]' : 'text-cyan-400 drop-shadow-[0_0_6px_rgba(34,211,238,0.6)] group-hover:scale-110'}`} />
            <Activity className={`w-3 h-3 transition-transform duration-200 ${pulseCheckActive ? 'text-slate-950 stroke-[2.5]' : 'text-amber-400 drop-shadow-[0_0_6px_rgba(251,191,36,0.6)] group-hover:scale-110'}`} />
          </div>
          <span className={`text-[9px] font-mono tracking-tight mt-0.5 ${pulseCheckActive ? 'text-slate-950 font-black' : 'text-cyan-200/90 font-bold group-hover:text-cyan-100'}`}>
            10s Check
          </span>
        </button>
      </div>
